...
function formatCurrency(amount?: number, currency?: "NGN" | "USD") {
  ...
  const overallSales = formatCurrency(salesSummary?.NGN?.overallSales, "NGN");
  const overallSalesDollar = formatCurrency(
    salesSummary?.USD?.overallSales,
    "USD"
  );
  const netPayout = formatCurrency(salesSummary?.NGN?.netPayout, "NGN");
  const netPayoutDollar = formatCurrency(salesSummary?.USD?.netPayout, "USD");

  // Choose the non-zero packageSold value, or sum if both are present
  const packagesSold =
    (salesSummary?.NGN?.packageSold || 0) +
    (salesSummary?.USD?.packageSold || 0);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 lg:h-[143vh] no-scrollbar overflow-y-scroll">
          <OverviewHeader
            ...
            overallSalesDollar={overallSalesDollar}
            netPayout={netPayout}
            netPayoutDollar={netPayoutDollar}
