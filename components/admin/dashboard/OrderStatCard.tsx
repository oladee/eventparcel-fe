...
const OrderStatCard: React.FC<Props> = ({ icon, label, value, delta, subtext }) => (
   ...
   <div id="stats-grid" className="">
        <div 
          ...
          <div 
            ...
            >
         <p id='stat-subtext' className={`mt-auto font-medium text-sm ${delta.startsWith("-") ? "text-red-500" : "text-green-500"}`}>{delta} <span className='text-[#718096]'>{subtext}</span></p> 
        </div>
        </div>
    </div>
)
