...
const CreatePackageModal: React.FC<CreatePackageModalProps> = ({ groudId, groupCurrency, setOpenModalPackage, mode, packageData }) => {
    ...
    const normalizeDeliveryArray = (raw: any): string[] => {
        ...
        if (typeof raw === "string") {
            ...
            arr = raw.map((s) => (typeof s === "string" ? s.trim() : ""));
        }
        // convert legacy values to new ones
        return arr.map((val) => {
            if (val === "homeDelivery:platformDelivery") return "platformDelivery";
            if (val === "homeDelivery:selfManaged") return "selfManaged";
            return val;
        }).filter(Boolean);
    };
    ...
    const [formData, setFormData] = useState<PackageFormData>({
        ...
        packageDelivery: normalizeDeliveryArray(packageData?.packageDelivery),
...
const validateField = useCallback(
    ...
    if (field === "packageSize") {
        ...
        const isPlatformDelivery = delivery.includes("platformDelivery") || delivery.includes("homeDelivery:platformDelivery");
        
        if (isPlatformDelivery && !stringValue.trim()) {
            return "Package size is required for platform delivery.";
        }
        
        // if you want to require size for other combos, add here
    }
...
[formData.packageDelivery]
    ...
    // unified selected options state (three independent toggles)
    const [selectedOptions, setSelectedOptions] = useState<{ platformDelivery: boolean; selfManaged: boolean; pickUp: boolean }>({
    ...
    useEffect(() => {
        ...
        setSelectedOptions({
            platformDelivery: packageOptions.includes("platformDelivery") || packageOptions.includes("homeDelivery:platformDelivery"),
            selfManaged: packageOptions.includes("selfManaged") || packageOptions.includes("homeDelivery:selfManaged"),
    ...
    const toggleDeliveryOption = (option: "pickUp" | "platformDelivery" | "selfManaged") => {
        ...
        const countSelections = (set: Set<string>) => {
            let cnt = 0;
            if (set.has("platformDelivery") || set.has("homeDelivery:platformDelivery")) cnt++;
            if (set.has("selfManaged") || set.has("homeDelivery:selfManaged")) cnt++;
            if (set.has("pickUp")) cnt++;
            return cnt;
        };

        setFormData((prev) => {
            const updatedSelections = new Set(prev.packageDelivery || []);
            const legacy = legacyFor(option);
            const isSelected = updatedSelections.has(option) || (legacy && updatedSelections.has(legacy));

            // Attempting to deselect
            if (isSelected) {
                // prevent deselecting if it's the only selection (min 1)
                if (countSelections(updatedSelections) <= 1) {
                    toast.info("Please select at least one delivery option.");
                    return prev;
                }

                updatedSelections.delete(option);
                if (legacy) updatedSelections.delete(legacy);
            } else {
                // Attempting to add
                // Enforce mutual exclusivity between platformDelivery and selfManaged
                if (option === "platformDelivery") {
                    updatedSelections.delete("selfManaged");
                    updatedSelections.delete("homeDelivery:selfManaged");
                } else if (option === "selfManaged") {
                    updatedSelections.delete("platformDelivery");
                    updatedSelections.delete("homeDelivery:platformDelivery");
                }

                const newCount = countSelections(updatedSelections) + 1; // adding this option
                if (newCount > 2) {
                    toast.info("You can select up to 2 delivery options only.");
                    return prev;
                }

                updatedSelections.add(option);
                if (legacy) updatedSelections.delete(legacy); // normalize
            }

            const newArray = Array.from(updatedSelections);

            // Sync UI toggles with normalized selections
            setSelectedOptions({
                platformDelivery: updatedSelections.has("platformDelivery") || updatedSelections.has("homeDelivery:platformDelivery"),
                selfManaged: updatedSelections.has("selfManaged") || updatedSelections.has("homeDelivery:selfManaged"),
                pickUp: updatedSelections.has("pickUp"),
            });

            return { ...prev, packageDelivery: newArray };
        });
    };
    ...
    delivery.includes("platformDelivery") ||
            ...
            <div className="w-[94vw] lg:w-[680px] max-h-[80vh] lg:max-h-[97vh] bg-[#FFFFFF] rounded-2xl shadow-lg px-5  md:p-5 flex flex-col overflow-y-auto">                {/* Header */}
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="flex-1 w-full">
                        ...
                        type="number" 
                        id="packageQuantity" 
                        value={formData.packageQuantity as any} 
                        onChange={handleChange} 
                ...
                <div className="flex flex-col gap-3 mt-2">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-col gap-2">
                            {/* Platform Delivery */}
                            <div
                                className="flex border border-gray-100 items-center rounded-[5px] gap-2 cursor-pointer p-2"
                                onClick={() => toggleDeliveryOption("platformDelivery")}
                            >
                                <Image
                                    src={selectedOptions.platformDelivery ? "/images/check.png" : "/images/unchecked.png"}
                                    alt="check"
                                    width={20}
                                    height={20}
                                />
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-sm text-[#718096]">We handle delivery for you</span>
                                </div>
                            </div>
                            ...
                            <div
                                className="flex border border-gray-100 items-center rounded-[5px] gap-2 cursor-pointer p-2"
                                onClick={() => toggleDeliveryOption("selfManaged")}
                            >
                                <Image
                                    src={selectedOptions.selfManaged ? "/images/check.png" : "/images/unchecked.png"}
                                    alt="check"
                                    width={20}
                                    height={20}
                                />
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-sm text-[#718096]">You handle delivery yourself</span>
                                </div>
                            </div>
