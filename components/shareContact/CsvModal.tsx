...
const CsvModal: React.FC<CsvModalProps> = ({ onClose }) => {
  ...
  const handleImport = async () => {
    ...
    // Optional: Client-side CSV parsing using PapaParse for preview/validation.
  ...
  if (contacts.length > 0) {
    return <ContactModal onClose={onClose} contacts={contacts} />;
  }
...
//     if (
//       file &&
//       (allowedMimeTypes.includes(file.type) || file.name.toLowerCase().endsWith(".csv"))
//     ) {
//       setSelectedFile(file);
//     } else {
//       toast.error("Please upload a valid CSV file.");
...
//               isDragActive ? "border-blue-500 bg-blue-50" : "border-gray-300"
//             }`}
//             aria-labelledby="uploadCSVHeader uploadCSVDesc"
//           >
//             {selectedFile ? (
...
//       console.error("Error uploading CSV:", error);
//       toast.error(
//         error.response?.data?.message || "An error occurred while uploading the CSV file."
//       );
//     } finally {
//       setLoading(false);
//     }
...
//         const parsedContacts = response.data.data.map(
//           (contact: any, index: number) => ({
//             id: index + 1,
//             name: contact.name,
//             phone: contact.phoneNumber,
...
//         <p id="downloadTemplate" className="mt-4 text-sm text-gray-900 font-medium">
//           Got a manually created CSV?{" "}
//           <a href="/Event_Parcel_CSV_TemplateNew.csv.csv"  download="Event_Parcel_CSV_TemplateNew.csv" className="text-primary cursor-pointer font-medium">
//             Download Template
//           </a>
//         </p>
